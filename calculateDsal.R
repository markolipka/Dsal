    ...
    for(t in Temp){
        for(s in Sal){
            Dsal.matrix <- rbind(Dsal.matrix, data.frame(Temp = t,
                                                         Sal = s,
                                                         Dsal = calculate.Dsal(el = element, temperature = t, salinity = s)$Dsal))
        }
    }
    
    library(viridis)
    ggplot(Dsal.matrix, aes(x = Temp, y = Sal, fill = Dsal, z = Dsal)) +
        geom_tile() +
        scale_fill_viridis() +
        ggtitle(element)
...
Dsal.element.ratios <- function(ely = "DIC", elx = "SO4"){
    ...
    library(viridis)
    ggplot(ratios, aes(x = Temp, y = Sal, fill = ratio, z = ratio)) +
        geom_tile() +
